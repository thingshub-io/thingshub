...
public abstract class ServiceLoaderHelper {
	...
	public static <T> Stream<T> findAll(Class<T> clazz, ClassLoader classLoader) {
		ServiceLoader<T> load = ServiceLoader.load(clazz, classLoader);
		return StreamSupport.stream(load.spliterator(), false);
	}
	...
	public static <T> Stream<T> findAll(Class<T> clazz, ClassLoader classLoader, Predicate<? super T> predicate) {
		ServiceLoader<T> load = ServiceLoader.load(clazz, classLoader);
		return StreamSupport.stream(load.spliterator(), false).filter(predicate);
	}
